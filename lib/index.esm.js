...
// 解析 JSON
function jsonParser(text, reviver) {
    if (typeof text === 'string') {
        try {
            return JSON.parse(text, reviver);
        }
        catch (error) {
            console.error("".concat(text, " is not a JSON"));
            return null;
        }
    }
    return null;
...
function getBrowser() {
    ...
    for (var i in browserList) {
        ...
        switch (i) {
            case 'Opera': {
                var res = UserAgent.match(/opera\/([\d.]+)/);
                if (res)
                    versions = res[1];
                break;
            }
            case 'IE10':
            case 'IE11': {
                var res = UserAgent.match(/(msie\s|trident.*rv:)([\w.]+)/);
                if (res)
                    versions = res[2];
                break;
            }
            case 'Firefox': {
                var res = UserAgent.match(/firefox\/([\d.]+)/);
                if (res)
                    versions = res[1];
                break;
            }
            case 'Edge': {
                var res = UserAgent.match(/edge\/([\d.]+)/);
                if (res)
                    versions = res[1];
                break;
            }
            case 'QQBrowser': {
                var res = UserAgent.match(/qqbrowser\/([\d.]+)/);
                if (res)
                    versions = res[1];
                break;
            }
            case 'UCBrowser': {
                var res = UserAgent.match(/ubrowser\/([\d.]+)/);
                if (res)
                    versions = res[1];
                break;
            }
            case 'Safari': {
                var res = UserAgent.match(/version\/([\d.]+)/);
                if (res)
                    versions = res[1];
                break;
            }
            case 'Chrome': {
                ...
                var res = UserAgent.match(/chrome\/([\d.]+)/);
                if (res)
                    versions = res[1];
                break;
            }
...
function findLatestNode(tree, cb, // 条件函数（参数item为当前的节点, 返回判断条件，结果为 true 则满足条件）
children) {
    ...
    try {
        for (var _i = 0, tree_1 = tree; _i < tree_1.length; _i++) {
            ...
            if (cb(item)) {
                ...
                var temp = findLatestNode(item[children], cb, children);
                if (temp) {
                    res = temp;
                    throw new Error('catch you');
                }
...
function collectLatestNodes(tree, cb, // cb 条件函数（返回判断条件，结果为 true 表示需要过略）
children) {
    if (children === void 0) { children = 'children'; }
    var stack = cloneDeep(tree); // 栈结构
    var currNode = null;
    var res = [];
    while (stack.length) {
        currNode = stack.pop();
        ...
        if (currNode[children]) {
            stack.push.apply(stack, currNode[children]);
        }
    }
    return res;
}
/*** @description 根据节点属性，过略某些节点，并返回原数组（原数组改变）*/
function filterNodes(tree, cb, // cb 条件函数（返回判断条件，结果为 true 表示需要过略）
children) {
    if (children === void 0) { children = 'children'; }
 ...
 */
function getNodesByProp(property, // 树节点对象（key-比对的属性 value-比对的值）
tree, attr) {
    var key = Object.keys(property)[0];
    var value = property[key];
    var result = [];
    var traverse = function (value, path, tree) {
        if (!tree.length)
            return;
...
export { NProgress, collectLatestNodes, downloadByUrl, filterNodes, findLatestNode, getBrowser, getNodesByProp, is, isArray$2 as isArray, isDef, isEmpty, isFunction$1 as isFunction, isNull, isNullOrUnDef, isNumber, isObject$1 as isObject, isString, isUnDef, jsonParser, searchParams };
